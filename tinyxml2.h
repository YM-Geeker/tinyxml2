...
/*
    ...
    内部工具类：主要负责各种底层通用操作，比如跳过空白字符、字符串比较、类型转换、UTF-8 字符处理
    主要特点：1.静态工具类(Static Utility Class)，所有成员函数都是static，意味着不需要创建对象即可使用。
             2.封装了一系列与 XML 文本处理、字符处理、类型转换等相关的 静态辅助功能；
*/
/*
    static静态函数：不需要通过类的实例调用，通过(类名::函数名)调用；
                   静态函数不能访问类的非静态成员 (成员变量/成员函数)，只能访问static修饰的成员函数。
*/
